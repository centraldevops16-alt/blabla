...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "Bulk restore archived S3 objects under a prefix and copy them to STANDARD_IA. "
            "Supports in-place overwrite or copy to a different prefix."
        )
    )
    p.add_argument("--bucket", required=True, help="S3 bucket name")
    p.add_argument("--source-prefix", required=True, help="Source prefix, for example archived/")
    p.add_argument("--dest-prefix", help="Destination prefix. Defaults to the same as source for in-place copy")
    ...
    p.add_argument("--include-non-archived", action="store_true", help="Also copy non-archived objects under the prefix to STANDARD_IA")
...
def restore_in_progress(meta: Dict) -> bool:
    return 'ongoing-request="true"' in restore_header(meta)
...
def map_destination_key(source_key: str, source_prefix: str, dest_prefix: str) -> str:
...
def main() -> int:
    ...
    args.dest_prefix = normalize_prefix(args.dest_prefix) if args.dest_prefix else args.source_prefix
    outdir = Path(args.output_dir)
    outdir.mkdir(parents=True, exist_ok=True)

    client = s3_client(args)

    eprint(f"[{now_utc()}] Listing objects under s3://{args.bucket}/{args.source_prefix}")
